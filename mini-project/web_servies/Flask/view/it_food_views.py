...
from Flask.models.models import italian_recipes
...
pklfile_me = r'C:\Users\PC\Desktop\AI_KDT6\KDT6\mini-project\web_servies\model\tae_bc_lstm_clf_model.pth'
...
# Blueprint instance
# http://127.0.0.1:5000/
it_food_bp = Blueprint('it_food', import_name='__name__', url_prefix='/', template_folder='templates')

# Routing Functions
# URL 처리
@it_food_bp.route('/food_it/', methods=['GET', 'POST'], endpoint='it')   
# endpoint: rul끝단. 플라스크에서의 의미 url의 끝단이 아닌 그걸 처리하는 함수의 별칭.
# 함수명을 외부에 노출 시키지 않을 수 있음. 내부적으로 함수명을 바꿀 수 있음.
def ko_food():
    food_list = italian_recipes.query.all()
    # 판별하기
    text=""
    msg ="난이도"
    ...
    if request.method == 'POST':
        ...
        if text:
            ...
            food_list = italian_recipes.query.filter(
                italian_recipes.feature.contains(text_me_str),
                italian_recipes.level_code == pred_me  # 레벨 체크
            ).all()
    ...
    return render_template('it_food.html', text=text, msg=msg, food_list=food_list)
