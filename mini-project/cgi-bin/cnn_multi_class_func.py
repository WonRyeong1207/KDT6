...
def make_dataset_cpu():
    ...
    train_datasets, val_datasets = random_split(
        train_datasets, [train_size_2, val_size]
    )
...
class DctDataset(Dataset):
    def __init__(self):
        ...
        train_datasets, test_datasets = random_split(
            img_datasets, [train_size, test_size]
        )
...
def predict_web(model, image_tensor):
    ...
    with torch.inference_mode():  # Ensure no gradients are computed
        ...
        predicted_animal = translate[
            labelDict[predicted_label]
        ]  # 인덱스를 통해 동물 이름 변환
...
def training_cuda(
    model, train_loader, val_loader, epochs, lr, batch_size=32, threshold=0.0001
):
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.inference_mode():
            with torch.no_grad():
                for val_images, val_labels in val_loader:
                    ...
                    total_val_loss += val_loss.item() * val_images.size(
                        0
                    )  # Accumulate loss
        ...
        if epoch % 5 == 0:
            print(
                f"[{epoch:5}/{epochs:5}]  [Train]         loss: {loss_train:.6f}, score: {score_val:.6f}"
            )
            print(
                f"[{epoch:5}/{epochs:5}]  [Validation]    loss: {loss_val:.6f}, score: {score_train:.6f}"
            )
...
def testing(model, test_images, test_labels):
    ...
    with torch.no_grad():
        ...
        acc = MulticlassAccuracy(num_classes=10).to(DEVICE)(pred, test_labels)
        f1 = MulticlassF1Score(num_classes=10).to(DEVICE)(pred, test_labels)
        mat = MulticlassConfusionMatrix(num_classes=10).to(DEVICE)(pred, test_labels)

    return loss, acc, f1, mat


def training(
    model, train_datasets, val_datasets, epochs=100, lr=0.001, batch_size=64, patience=5
):
    loss_dict = {"train": [], "val": []}
    acc_dict = {"train": [], "val": []}
    f1_dict = {"train": [], "val": []}
    ...
    for epoch in range(1, epochs + 1):
        ...
        for images, labels in train_data_dl:
            ...
            acc = MulticlassAccuracy(num_classes=10).to(DEVICE)(pred, labels)
            total_t_acc += acc

            score = MulticlassF1Score(num_classes=10).to(DEVICE)(pred, labels)
            total_t_f1 += score
        ...
        if epoch % 5 == 0:
            print("---check point---")
            print(
                f"[{epoch:5}/{epochs:5}]  [Train]       loss: {train_loss:.6f}, score: {train_score*100:.4f} %"
            )
            print(
                f"[{epoch:5}/{epochs:5}]  [Validation]  loss: {val_loss:.6f}, score: {val_score*100:.4f} %\n"
            )
        ...
        if scheduler.num_bad_epochs >= scheduler.patience:
            ...
            print(
                f"[{epoch:5}/{epochs:5}]  [Train] accuracy: {train_acc*100:.4f} %, [Validation] accuracy:{val_acc*100:.4f} %"
            )
            break
...
def draw_two_plot(loss, r2, title):
    ...
    ax1.plot(
        loss["train"],
        label=f"train loss mean: {sum(loss['train'])/len(loss['train']):.6f}",
        color="#5587ED",
    )
    ax1.plot(
        loss["val"],
        label=f"validation loss mean: {sum(loss['val'])/len(loss['val']):.6f}",
        color="#F361A6",
    )
    ax2.plot(
        r2["train"],
        label=f"train score max: {max(r2['train'])*100:.2f} %",
        color="#00007F",
    )
    ax2.plot(
        r2["val"],
        label=f"validation score max: {max(r2['val'])*100:.2f} %",
        color="#99004C",
    )
